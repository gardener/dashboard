...
function attach (options = {}) {
  const LEVELS = globalLogger.LEVELS;
  if (!globalLogger.isDisabled(LEVELS.debug)) {
...
class Reflector {
  ...
  async run (signal) {
    ...
    globalLogger.info('Starting reflector %s', this.expectedTypeName);
    while (!this.signal.aborted) {
      try {
        await this.listAndWatch();
      } catch (err) {
        if (!isAbortError(err)) {
          globalLogger.error('Failed to list and watch %s: %s', this.expectedTypeName, err);
        }
      }
      ...
      globalLogger.info('Restarting reflector %s', this.expectedTypeName);
    }
    globalLogger.info('Stopped reflector %s', this.expectedTypeName);
  }
  ...
  async listAndWatch () {
    ...
    let list;
    try {
      globalLogger.debug('List %s with resourceVersion %s', this.expectedTypeName, options.resourceVersion);
      ...
      if (isExpiredError(err) || isTooLargeResourceVersionError(err)) {
        ...
        try {
          globalLogger.debug('Falling back to full list %s', this.expectedTypeName);
          list = await pager.list({
            resourceVersion: this.relistResourceVersion,
          });
        } catch (err) {
          globalLogger.error('Failed to call full list %s: %s', this.expectedTypeName, err.message);
          return
        }
      }
      globalLogger.error('Failed to call paginated list %s: %s', this.expectedTypeName, err.message);
      return
    }
    ...
    globalLogger.debug('List of %s successfully returned %d items (%s)', this.expectedTypeName, lines, paginatedResult ? 'paginated' : 'not paginated');
    ...
    while (!this.signal.aborted) {
      ...
      let response;
      try {
        globalLogger.debug('Watch %s with resourceVersion %s', this.expectedTypeName, options.resourceVersion);
        ...
        if (isConnectionRefused(err)) {
          ...
          globalLogger.info('Watch of %s refused connection with: %s', this.expectedTypeName, err.message);
      ...
      try {
        ...
        if (isExpiredError(err)) {
          ...
          globalLogger.info('Watch of %s closed with: %s', this.expectedTypeName, err.message);
        } else if (!isAbortError(err)) {
          globalLogger.warn('Watch of %s ended with: %s', this.expectedTypeName, err.message);
        }
        return
      }
    }
  ...
  async watchHandler (response, timeout) {
    ...
    try {
      for await (const event of response) {
        ...
        if (apiVersion !== this.apiVersion || kind !== this.kind) {
          ...
          globalLogger.error('Expected %s, but watch event object had %s', this.expectedTypeName, typeName);
          continue
        }
        switch (type) {
          case 'ADDED':
          ...
          case 'BOOKMARK':
            break
          default:
            globalLogger.error('Unable to understand event %s for watch %s', type, this.expectedTypeName);
        }
        if (resourceVersion) {
          this.lastSyncResourceVersion = resourceVersion;
        } else {
          globalLogger.error('Received event object without resource version for watch %s', this.expectedTypeName);
        }
      }
    } finally {
      clearTimeout(timeoutId);
    ...
    if (duration < 1000 && count === 0) {
      throw new Error(format('Very short watch %s - watch lasted less than a second and no items received', this.expectedTypeName))
    }
    globalLogger.info('Watch %s closed - total %d items received within %s seconds', this.expectedTypeName, count, Math.floor(duration / 1000));
  }
...
const CoreGroup = superclass => class extends superclass {
  static get [http.relativeUrl] () {
    return join$1('api', this.version)
  }
};

const NamedGroup = superclass => class extends superclass {
  static get [http.relativeUrl] () {
    return join$1('apis', this.group, this.version)
  }
};
  ...
  async getKubeconfig ({ name, namespace }) {
    ...
    if (serviceaccountJsonBase64) {
      try {
        ...
        globalLogger.warn(`Failed to refresh auth-provider config of kubeconfig secret "${name}" in namespace "${namespace}"`);
      }
    }
    return kubeconfig
  }
