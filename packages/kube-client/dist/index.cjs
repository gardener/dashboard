...
var request = require('@gardener-dashboard/request');
...
var kubeConfig = require('@gardener-dashboard/kube-config');
 ...
 */
var delay$1 = baseRest(function(func, wait, args) {
...
var func = {
  ...
  curry, curryRight, debounce, defer, delay: delay$1,
...
var object = {
  assign: assign$2, assignIn, assignInWith, assignWith, at,
...
function setContentType (options, value) {
  return lodash.set(options, ['headers', HTTP2_HEADER_CONTENT_TYPE], value)
}
...
function getUser ({ headers, key, cert }) {
  ...
  switch (schema) {
    ...
    case 'Basic': {
      user.type = 'user';
      user.id = head(split(decodeBase64(value), ':'));
      break
    }
  }
  ...
  if (key && cert) {
    try {
      const rCRLF = /\r?\n/;
      const x509 = new require$$2.X509Certificate(cert);
...
function attach (options = {}) {
  const LEVELS = logger.globalLogger.LEVELS;
  if (!logger.globalLogger.isDisabled(LEVELS.debug)) {
...
var hasRequiredMixwith;

function requireMixwith () {
	if (hasRequiredMixwith) return mixwith;
	hasRequiredMixwith = 1;
	(function (exports) {
		...
		(function (global, factory) {
		  ...
		  exports.Mixin = mixin => Cached(HasInstance(BareMixin(mixin)));
...
var mixwithExports = requireMixwith();
...
const { isHttpError: isHttpError$1 } = httpErrors;
...
class Reflector {
  ...
  async run (signal) {
    assertSignal$1(signal);
    this.setAbortSignal(signal);
    logger.globalLogger.info('Starting reflector %s', this.expectedTypeName);
    while (!this.signal.aborted) {
      try {
        await this.listAndWatch();
      } catch (err) {
        if (!isAbortError(err)) {
          logger.globalLogger.error('Failed to list and watch %s: %s', this.expectedTypeName, err);
        }
      }
      ...
      logger.globalLogger.info('Restarting reflector %s', this.expectedTypeName);
    }
    logger.globalLogger.info('Stopped reflector %s', this.expectedTypeName);
  }
  ...
  async listAndWatch () {
    ...
    if (this.paginatedResult) ; else if (options.resourceVersion !== '' && options.resourceVersion !== '0') {
    ...
    let list;
    try {
      logger.globalLogger.debug('List %s with resourceVersion %s', this.expectedTypeName, options.resourceVersion);
      ...
      if (isExpiredError(err) || isTooLargeResourceVersionError(err)) {
        ...
        try {
          logger.globalLogger.debug('Falling back to full list %s', this.expectedTypeName);
          list = await pager.list({
            resourceVersion: this.relistResourceVersion,
          });
        } catch (err) {
          logger.globalLogger.error('Failed to call full list %s: %s', this.expectedTypeName, err.message);
          return
        }
      }
      logger.globalLogger.error('Failed to call paginated list %s: %s', this.expectedTypeName, err.message);
      return
    }
    ...
    logger.globalLogger.debug('List of %s successfully returned %d items (%s)', this.expectedTypeName, lines, paginatedResult ? 'paginated' : 'not paginated');
    ...
    while (!this.signal.aborted) {
      ...
      let response;
      try {
        logger.globalLogger.debug('Watch %s with resourceVersion %s', this.expectedTypeName, options.resourceVersion);
        ...
        if (isConnectionRefused(err)) {
          ...
          logger.globalLogger.info('Watch of %s refused connection with: %s', this.expectedTypeName, err.message);
      ...
      try {
        ...
        if (isExpiredError(err)) {
          ...
          logger.globalLogger.info('Watch of %s closed with: %s', this.expectedTypeName, err.message);
        } else if (!isAbortError(err)) {
          logger.globalLogger.warn('Watch of %s ended with: %s', this.expectedTypeName, err.message);
        }
        return
      }
    }
  ...
  async watchHandler (response, timeout) {
    ...
    try {
      for await (const event of response) {
        ...
        if (apiVersion !== this.apiVersion || kind !== this.kind) {
          ...
          logger.globalLogger.error('Expected %s, but watch event object had %s', this.expectedTypeName, typeName);
          continue
        }
        switch (type) {
          case 'ADDED':
          ...
          case 'BOOKMARK':
            break
          default:
            logger.globalLogger.error('Unable to understand event %s for watch %s', type, this.expectedTypeName);
        }
        if (resourceVersion) {
          this.lastSyncResourceVersion = resourceVersion;
        } else {
          logger.globalLogger.error('Received event object without resource version for watch %s', this.expectedTypeName);
        }
      }
    } finally {
      clearTimeout(timeoutId);
    ...
    if (duration < 1000 && count === 0) {
      throw new Error(node_util.format('Very short watch %s - watch lasted less than a second and no items received', this.expectedTypeName))
    }
    logger.globalLogger.info('Watch %s closed - total %d items received within %s seconds', this.expectedTypeName, count, Math.floor(duration / 1000));
  }
...
function assertSignal$1 (signal) {
...
const CoreGroup = superclass => class extends superclass {
  static get [http.relativeUrl] () {
    return path.join('api', this.version)
  }
};

const NamedGroup = superclass => class extends superclass {
  static get [http.relativeUrl] () {
    return path.join('apis', this.group, this.version)
  }
};
...
const NamespaceScoped = mixwithExports.Mixin(superclass => class extends superclass {
  static get scope () {
    return 'Namespaced'
  }
});

const ClusterScoped = mixwithExports.Mixin(superclass => class extends superclass {
  static get scope () {
    return 'Cluster'
  }
});
...
const Readable = mixwithExports.Mixin(superclass => {
...
const Observable = mixwithExports.Mixin(superclass => {
  switch (superclass.scope) {
    case 'Cluster':
...
const Creatable = mixwithExports.Mixin(superclass => {
  switch (superclass.scope) {
    case 'Cluster':
...
const Writable = mixwithExports.Mixin(superclass => {
  switch (superclass.scope) {
    case 'Cluster':
...
let APIRegistration$1 = class APIRegistration extends V1(NamedGroup(HttpClient)) {
  static get group () {
    return 'apiregistration.k8s.io'
  }
};

let Authentication$1 = class Authentication extends V1(NamedGroup(HttpClient)) {
  static get group () {
    return 'authentication.k8s.io'
  }
};

let Authorization$1 = class Authorization extends V1(NamedGroup(HttpClient)) {
  static get group () {
    return 'authorization.k8s.io'
  }
};

let Coordination$1 = class Coordination extends V1(NamedGroup(HttpClient)) {
  static get group () {
    return 'coordination.k8s.io'
  }
};

let Core$1 = class Core extends V1(CoreGroup(HttpClient)) {
  static get group () {
    return ''
  }
};

let Networking$1 = class Networking extends V1(NamedGroup(HttpClient)) {
  static get group () {
    return 'networking.k8s.io'
  }
};

let GardenerCore$1 = class GardenerCore extends V1Beta1(NamedGroup(HttpClient)) {
  static get group () {
    return 'core.gardener.cloud'
  }
};

let GardenerSeedManagement$1 = class GardenerSeedManagement extends V1Alpha1(NamedGroup(HttpClient)) {
  static get group () {
    return 'seedmanagement.gardener.cloud'
  }
};

let GardenerDashboard$1 = class GardenerDashboard extends V1Alpha1(NamedGroup(HttpClient)) {
  static get group () {
    return 'dashboard.gardener.cloud'
  }
};

let GardenerSecurity$1 = class GardenerSecurity extends V1Alpha1(NamedGroup(HttpClient)) {
  static get group () {
...
var groups = /*#__PURE__*/Object.freeze({
  __proto__: null,
  APIRegistration: APIRegistration$1,
  Authentication: Authentication$1,
  Authorization: Authorization$1,
  Coordination: Coordination$1,
  Core: Core$1,
  GardenerCore: GardenerCore$1,
  GardenerDashboard: GardenerDashboard$1,
  GardenerSecurity: GardenerSecurity$1,
  GardenerSeedManagement: GardenerSeedManagement$1,
...
class APIService extends mixwithExports.mix(APIRegistration$1).with(ClusterScoped, Readable) {
  static get names () {
    return {
      plural: 'apiservices',
      singular: 'apiservice',
...
class TokenReview extends mixwithExports.mix(Authentication$1).with(ClusterScoped, Creatable) {
  static get names () {
    return {
      plural: 'tokenreviews',
...
class SelfSubjectAccessReview extends mixwithExports.mix(Authorization$1).with(ClusterScoped, Creatable) {
  static get names () {
    return {
...
class SelfSubjectRulesReview extends mixwithExports.mix(Authorization$1).with(ClusterScoped, Creatable) {
  static get names () {
    return {
...
class SubjectAccessReview extends mixwithExports.mix(Authorization$1).with(ClusterScoped, Creatable) {
  static get names () {
    return {
...
class Lease extends mixwithExports.mix(Coordination$1).with(NamespaceScoped, Readable, Observable, Writable) {
  static get names () {
    return {
      plural: 'leases',
      singular: 'lease',
...
class Endpoints extends mixwithExports.mix(Core$1).with(NamespaceScoped, Readable, Writable) {
  static get names () {
    return {
      plural: 'endpoints',
      singular: '',
...
class Namespace extends mixwithExports.mix(Core$1).with(ClusterScoped, Readable, Observable) {
  static get names () {
    return {
      plural: 'namespaces',
      singular: 'namespace',
...
class Node extends mixwithExports.mix(Core$1).with(ClusterScoped, Readable, Observable) {
  static get names () {
    return {
      plural: 'nodes',
      singular: 'node',
...
class Pod extends mixwithExports.mix(Core$1).with(NamespaceScoped, Readable, Observable, Writable) {
  static get names () {
    return {
      plural: 'pods',
      singular: 'pod',
...
class ConfigMap extends mixwithExports.mix(Core$1).with(NamespaceScoped, Readable) {
  static get names () {
    return {
      plural: 'configmaps',
      singular: 'configmap',
...
class Secret extends mixwithExports.mix(Core$1).with(NamespaceScoped, Readable, Observable, Writable) {
  static get names () {
    return {
      plural: 'secrets',
      singular: 'secret',
...
class Service extends mixwithExports.mix(Core$1).with(NamespaceScoped, Readable, Observable, Writable) {
  static get names () {
    return {
      plural: 'services',
      singular: 'service',
...
class ServiceAccount extends mixwithExports.mix(Core$1).with(NamespaceScoped, Readable, Observable, Writable) {
...
class ResourceQuota extends mixwithExports.mix(Core$1).with(NamespaceScoped, Readable, Observable) {
  static get names () {
    return {
...
class Project extends mixwithExports.mix(GardenerCore$1).with(ClusterScoped, Readable, Observable, Writable) {
  static get names () {
    return {
      plural: 'projects',
      singular: 'project',
...
class CloudProfile extends mixwithExports.mix(GardenerCore$1).with(ClusterScoped, Readable, Observable) {
  static get names () {
    return {
...
class Seed extends mixwithExports.mix(GardenerCore$1).with(ClusterScoped, Readable, Observable) {
  static get names () {
    return {
      plural: 'seeds',
      singular: 'seed',
...
class Quota extends mixwithExports.mix(GardenerCore$1).with(NamespaceScoped, Readable, Observable) {
  static get names () {
    return {
      plural: 'quotas',
      singular: 'quota',
...
class SecretBinding extends mixwithExports.mix(GardenerCore$1).with(NamespaceScoped, Readable, Observable, Writable) {
  static get names () {
    return {
...
class Shoot extends mixwithExports.mix(GardenerCore$1).with(NamespaceScoped, Readable, Observable, Writable) {
...
class ControllerRegistration extends mixwithExports.mix(GardenerCore$1).with(ClusterScoped, Readable, Observable) {
  static get names () {
    return {
...
class Terminal extends mixwithExports.mix(GardenerDashboard$1).with(NamespaceScoped, Readable, Observable, Writable) {
  static get names () {
    return {
      plural: 'terminals',
      singular: 'terminal',
...
class SeedManagement extends mixwithExports.mix(GardenerSeedManagement$1).with(NamespaceScoped, Readable, Observable, Writable) {
  static get names () {
    return {
      plural: 'managedseeds',
...
class Ingress extends mixwithExports.mix(Networking$1).with(NamespaceScoped, Readable, Observable, Writable) {
  static get names () {
    return {
      plural: 'ingresses',
      singular: 'ingress',
...
class CredentialsBinding extends mixwithExports.mix(GardenerSecurity$1).with(NamespaceScoped, Readable, Observable, Writable) {
  static get names () {
    return {
...
class WorkloadIdentity extends mixwithExports.mix(GardenerSecurity$1).with(NamespaceScoped, Readable, Observable, Writable) {
  static get names () {
    return {
...
function loadGroup ({ name }) {
  ...
  return lodash.mapKeys(resources, 'names.plural')
}

function load$2 (clientConfig, options) {
  const createInstance = Ctor => new Ctor(clientConfig.extend({
    ...options,
    responseType: 'json',
    relativeUrl: Ctor[http.relativeUrl],
  }));
  const createInstances = resourceGroup => lodash.mapValues(resourceGroup, createInstance);
  return lodash.mapValues(resourceGroups, createInstances)
}
...
const Resources = lodash.reduce(resourceGroups, getResourceGroupMetadata, {
  TokenRequest: {
    name: 'token',
    kind: 'TokenRequest',
...
function assign$1 (object, ...args) {
  return Object.assign(object, load$2(...args))
}
...
function load$1 (clientConfig, options) {
  const createInstance = ({ Ctor, ...rest }) => new Ctor(clientConfig.extend({
    ...options,
    ...rest,
    relativeUrl: Ctor[http.relativeUrl],
  }));
  return lodash.mapValues(endpoints, createInstance)
}

function assign (object, ...args) {
  return Object.assign(object, load$1(...args))
}
...
class Client {
  ...
  constructor (clientConfig, options) {
    ...
    assign$1(this, clientConfig, options);
    // assign non-resource endpoints (e.g healthz)
    assign(this, clientConfig, options);
  }
  ...
  async getKubeconfig ({ name, namespace }) {
    ...
    const kubeconfigBase64 = lodash.get(secret, ['data', 'kubeconfig']);
    if (!kubeconfigBase64) {
      throw NotFound('No "kubeconfig" found in secret')
    }
    const kubeconfig = parseKubeconfig(decodeBase64$1(kubeconfigBase64));
    const authProviderName = lodash.get(kubeconfig, ['currentUser', 'auth-provider', 'name']);
    const serviceaccountJsonBase64 = lodash.get(secret, ['data', 'serviceaccount.json']);
    ...
    if (serviceaccountJsonBase64) {
      try {
        const credentials = JSON.parse(decodeBase64$1(serviceaccountJsonBase64));
        await kubeconfig.refreshAuthProviderConfig(credentials);
      } catch (err) {
        logger.globalLogger.warn(`Failed to refresh auth-provider config of kubeconfig secret "${name}" in namespace "${namespace}"`);
      }
    }
    return kubeconfig
  }
  ...
  async createShootAdminKubeconfig ({ name, namespace }) {
    ...
    const kubeconfigBase64 = lodash.get(adminKubeconfigRequest, ['status', 'kubeconfig']);
    return parseKubeconfig(decodeBase64$1(kubeconfigBase64))
  }
