...
async function createWatch (paths, options = {}) {
  ...
  const abort = () => {
    ...
    aborted = true;
    globalLogger.info('[polling-watcher] watch aborted');
    clearInterval(intervalId);
    removeAbortListener(signal, abort);
  };

  const hashes = await createHashes(paths);
  globalLogger.info('[polling-watcher] watch created for %d files', paths.length);
  ...
  return fn => {
    const tryReadFile = async path => {
      try {
        ...
        if (newHash !== oldHash) {
          globalLogger.info('[polling-watcher] detected file change `%s`', path);
          hashes.set(path, newHash);
          fn(path, data.toString('utf8'));
        }
      } catch (err) {
        globalLogger.error('[polling-watcher] failed to read file `%s`: %s', path, err.message);
      }
    };
    ...
    globalLogger.debug('[polling-watcher] watch started with interval %dms', interval);

    return abort
  }
}

export { createWatch as default };
