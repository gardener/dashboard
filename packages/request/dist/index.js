...
class SessionPool {
  ...
  async request (headers, options) {
    ...
    globalLogger.trace('Session %s - stream aquired (%d/%d)', this.id, concurrency, semaphore.maxConcurrency);
    const releaseStream = () => {
      release();
      globalLogger.trace('Session %s - stream released (%d/%d)', this.id, semaphore.concurrency, semaphore.maxConcurrency);
    };
    ...
    try {
      ...
      if (stream.pending) {
        stream.once('ready', () => {
          globalLogger.trace('Session %s - stream %d emitted "ready"', this.id, stream.id);
        });
      } else {
        globalLogger.trace('Session %s - stream %d ready', this.id, stream.id);
      }
      stream.once('finish', () => {
        globalLogger.trace('Session %s - stream %d emitted "finish"', this.id, stream.id || -1);
      });
      const headersPromise = new Promise((resolve, reject) => {
        let settled = false;
        stream.on('error', err => {
          if (!isAbortError(err)) {
            globalLogger.error('Session %s - stream %d processing error: %s', this.id, stream.id || -1, err.message);
          }
          if (!settled) {
            settled = true;
            reject(err);
        ...
        stream.once('close', () => {
          globalLogger.trace('Session %s - stream %d closed', this.id, stream.id || -1);
          releaseStream();
          const { [kSemaphore]: semaphore } = session;
        ...
        stream.once('response', headers => {
          globalLogger.trace('Session %s - stream %d emitted "response"', this.id, stream.id);
          if (!settled) {
            settled = true;
            resolve(headers);
          }
      ...
      globalLogger.error('Session %s - stream creation error: %s', this.id, err.message);
      releaseStream();
      throw err
    }
  }
  ...
  setSessionHeartbeat (session) {
    ...
    const cancel = err => {
      if (!canceled) {
        canceled = true;
        globalLogger.debug('Session %s - canceled: %s', this.id, err.message);
    ...
    const ping = () => {
      try {
        ...
        globalLogger.error('Session %s - heartbeat error: %s', this.id, err.message);
    ...
    const pong = (err, duration) => {
      if (err) {
        ...
        globalLogger.trace('Session %s - ping %d ms', this.id, Math.round(duration));
      }
    };
    globalLogger.debug('Session %s - starting heartbeat with %ds ping interval', this.id, Math.floor(this.pingInterval / 1000));
  ...
  createSession () {
    ...
    globalLogger.debug('Session %s - pool size is %d (scaled up)', this.id, this.sessions.size);
    // tls session can be used for resumption
    session.socket.once('session', session => {
      globalLogger.debug('Session %s - received tls session after %d ms', this.id, timer.duration());
    ...
    const setMaxConcurrency = settings => {
      ...
      if (oldValue !== newValue) {
        ...
        globalLogger.debug('Session %s - maximum concurrency %s to %d', this.id, change, newValue);
    ...
    session.once('error', err => {
      globalLogger.error('Session %s - connect error: %s', this.id, err.message);
      clearTimeout(timeoutId);
      session.removeAllListeners('connect');
      this.deleteSession(session);
      this.tlsSession = undefined;
    ...
    session.once('connect', () => {
      globalLogger.debug('Session %s - connected after %d ms', this.id, timer.duration());
      clearTimeout(timeoutId);
      session.removeAllListeners('error');
      session.on('error', err => {
        globalLogger.error('Session %s - unexpected error: %s', this.id, err.message);
        this.tlsSession = undefined;
      });
      session.once('close', () => {
        globalLogger.info('Session %s - closed after %d ms', this.id, timer.duration());
        this.deleteSession(session);
      });
  ...
  deleteSession (session, ...args) {
    ...
    globalLogger.debug('Session %s - pool size is %d (scaled down)', this.id, this.sessions.size);
    // destory session
...
class Agent {
  ...
  getSessionPool (sid) {
    ...
    if (!sessionPool) {
      ...
      globalLogger.debug('Agent - map size is %d (pool %s added)', this.sessionPools.size, sid);
    }
    return sessionPool
  }
}
...
class Client {
  ...
  executeHooks (name, ...args) {
    try {
      ...
      globalLogger.error(`Failed to execute "${name}" hooks`, err.message);
    }
  }
            ...
            switch (this.type) {
              ...
              case 'json':
                try {
                  ...
                  globalLogger.error('Failed to parse response body: %s', text);
                  if (this.ok) {
      ...
      async * [Symbol.asyncIterator] () {
        ...
        if (decompressor) {
          readable = pipeline(stream, decompressor, err => {
            if (err) {
              globalLogger.debug('Stream decompress pipeline error: %s', err.message);
            }
          });
        }
