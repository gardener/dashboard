...
class SessionPool {
  ...
  async request (headers, options) {
    ...
    logger.globalLogger.trace('Session %s - stream aquired (%d/%d)', this.id, concurrency, semaphore.maxConcurrency);
    const releaseStream = () => {
      release();
      logger.globalLogger.trace('Session %s - stream released (%d/%d)', this.id, semaphore.concurrency, semaphore.maxConcurrency);
    };
    ...
    try {
      ...
      if (stream.pending) {
        stream.once('ready', () => {
          logger.globalLogger.trace('Session %s - stream %d emitted "ready"', this.id, stream.id);
        });
      } else {
        logger.globalLogger.trace('Session %s - stream %d ready', this.id, stream.id);
      }
      stream.once('finish', () => {
        logger.globalLogger.trace('Session %s - stream %d emitted "finish"', this.id, stream.id || -1);
      });
      const headersPromise = new Promise((resolve, reject) => {
        let settled = false;
        stream.on('error', err => {
          if (!isAbortError(err)) {
            logger.globalLogger.error('Session %s - stream %d processing error: %s', this.id, stream.id || -1, err.message);
          }
          if (!settled) {
            settled = true;
            reject(err);
        ...
        stream.once('close', () => {
          logger.globalLogger.trace('Session %s - stream %d closed', this.id, stream.id || -1);
          releaseStream();
          const { [kSemaphore]: semaphore } = session;
        ...
        stream.once('response', headers => {
          logger.globalLogger.trace('Session %s - stream %d emitted "response"', this.id, stream.id);
          if (!settled) {
            settled = true;
            resolve(headers);
          }
      ...
      logger.globalLogger.error('Session %s - stream creation error: %s', this.id, err.message);
      releaseStream();
      throw err
    }
  }
  ...
  setSessionHeartbeat (session) {
    ...
    const cancel = err => {
      if (!canceled) {
        canceled = true;
        logger.globalLogger.debug('Session %s - canceled: %s', this.id, err.message);
    ...
    const ping = () => {
      try {
        ...
        logger.globalLogger.error('Session %s - heartbeat error: %s', this.id, err.message);
    ...
    const pong = (err, duration) => {
      if (err) {
        ...
        logger.globalLogger.trace('Session %s - ping %d ms', this.id, Math.round(duration));
      }
    };
    logger.globalLogger.debug('Session %s - starting heartbeat with %ds ping interval', this.id, Math.floor(this.pingInterval / 1000));
  ...
  createSession () {
    ...
    logger.globalLogger.debug('Session %s - pool size is %d (scaled up)', this.id, this.sessions.size);
    // tls session can be used for resumption
    session.socket.once('session', session => {
      logger.globalLogger.debug('Session %s - received tls session after %d ms', this.id, timer.duration());
    ...
    const setMaxConcurrency = settings => {
      ...
      if (oldValue !== newValue) {
        ...
        logger.globalLogger.debug('Session %s - maximum concurrency %s to %d', this.id, change, newValue);
    ...
    session.once('error', err => {
      logger.globalLogger.error('Session %s - connect error: %s', this.id, err.message);
      clearTimeout(timeoutId);
      session.removeAllListeners('connect');
      this.deleteSession(session);
      this.tlsSession = undefined;
    ...
    session.once('connect', () => {
      logger.globalLogger.debug('Session %s - connected after %d ms', this.id, timer.duration());
      clearTimeout(timeoutId);
      session.removeAllListeners('error');
      session.on('error', err => {
        logger.globalLogger.error('Session %s - unexpected error: %s', this.id, err.message);
        this.tlsSession = undefined;
      });
      session.once('close', () => {
        logger.globalLogger.info('Session %s - closed after %d ms', this.id, timer.duration());
        this.deleteSession(session);
      });
  ...
  deleteSession (session, ...args) {
    ...
    logger.globalLogger.debug('Session %s - pool size is %d (scaled down)', this.id, this.sessions.size);
    // destory session
...
class Agent {
  ...
  request (headers, options = {}) {
    ...
    const {
      [HTTP2_HEADER_SCHEME$1]: scheme = 'https',
      [HTTP2_HEADER_AUTHORITY$1]: authority,
  ...
  getSessionPool (sid) {
    ...
    if (!sessionPool) {
      ...
      logger.globalLogger.debug('Agent - map size is %d (pool %s added)', this.sessionPools.size, sid);
    }
    return sessionPool
  }
}
...
class Client {
  ...
  constructor (options = {}) {
    ...
    this.#agent = options.agent || globalAgent$1;
  ...
  get #defaultOptions () {
    ...
    const servername = net.isIP(hostname) !== 0 ? '' : hostname;
    const keys = [
      'ca',
      'rejectUnauthorized',
      'key',
  ...
  executeHooks (name, ...args) {
    try {
      ...
      logger.globalLogger.error(`Failed to execute "${name}" hooks`, err.message);
    }
  }
  ...
  getRequestHeaders (path = '', { method = HTTP2_METHOD_GET, searchParams, headers } = {}) {
    ...
    if (pathname.startsWith('/')) {
      ...
      url.pathname = require$$1.join(url.pathname, pathname);
    }
    if (!searchParams) {
      url.search = search;
  ...
  async fetch (path, { method, searchParams, headers, body, responseType = this.responseType, signal, ...options } = {}) {
    ...
    const stream$1 = await this.#agent.request(headers, {
      ...this.#defaultOptions,
      ...options,
      signal,
    });
    ...
    headers = await stream$1.getHeaders();
    ...
    return {
      ...
      body () {
            ...
            switch (this.type) {
              ...
              case 'json':
                try {
                  ...
                  logger.globalLogger.error('Failed to parse response body: %s', text);
                  if (this.ok) {
        ...
        return new Promise((resolve, reject) => {
          stream.pipeline(streams, (err, body) => {
            if (err) {
              reject(err);
            } else {
              resolve(body);
      ...
      async * [Symbol.asyncIterator] () {
        ...
        let readable = stream$1;
        if (decompressor) {
          readable = stream.pipeline(stream$1, decompressor, err => {
            if (err) {
              logger.globalLogger.debug('Stream decompress pipeline error: %s', err.message);
            }
          });
        }
