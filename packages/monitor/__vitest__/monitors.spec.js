...
async function spyOnHelper (moduleName, methodPaths, importMetaUrl = '') {
  ...
  const { default: actual } = await vi.importActual(moduleName)
  ...
  for (const method of methodPaths) {
    if (method.includes('.')) {
      ...
      parent[property] = vi.fn(parent[property])
    } else {
      return {
...
describe('monitors', () => {
  describe('socketIO connection monitor', () => {
    const ioStub = { on: vi.fn() }
    const socketStub = { once: vi.fn() }
