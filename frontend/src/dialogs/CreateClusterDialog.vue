...
const standardAddonDefinitionList = [
  ...
  },
  {
    ...
    description: 'Metallb Loadbalancer to provide service type Loadbalancer',
    visible: true,
    enabled: true
  }
]
...
export default {
  ...
  computed: {
    ...
    sortedCloudProviderKindList () {
      return intersection(['aws', 'azure', 'gcp', 'openstack', 'alicloud', 'metal'], this.cloudProviderKindList)
    },
    projectName () {
  ...
  methods: {
    ...
    createShootResource () {
      ...
      // FIXME metal specific
      const networkFromZone = {
            'fra-equ01': [
              'internet-fra-equ01',
            ],
            'nbg-w8101': [
      ...
      console.log("Zone: ", data , " InfrastructureData: ", infrastructureData)
      ...
      data.spec.cloud.metal.networks.additional = networkFromZone[infrastructureData.zones[0]]
      data.spec.cloud.metal.loadBalancerProvider = "metallb"
      ...
      // FIXME metal specific
      data.spec.addons['metallb'] = {
        'enabled': true,
        'networks': [
          {
            'name': networkFromZone[infrastructureData.zones[0]][0],
            'count': 2
          },
        ],
      }
