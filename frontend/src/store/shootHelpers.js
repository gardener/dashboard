...
export function findItem (list) {
  return ({ name, namespace }) => list[keyForShoot({ name, namespace })]
}

export function putItem (list, newItem) {
  const item = findItem(list)(newItem.metadata)
  if (item) {
    if (item.metadata.resourceVersion !== newItem.metadata.resourceVersion) {
      list[keyForShoot(item.metadata)] = assign(item, newItem)
    }
  } else {
    if (list.focusMode) {
      delete list.staleShoots[newItem.metadata.uid]
    }
    newItem.info = undefined // register property to ensure reactivity
    list[keyForShoot(newItem.metadata)] = newItem
  }
}
export function deleteItem (list, deletedItem) {
  const item = findItem(list)(deletedItem.metadata)

  if (item) {
    if (list.focusMode) {
      list.staleShoots[item.metadata.uid] = item
    }
    delete list[keyForShoot(item.metadata)]
  }
}
...
const wellKnownConditions = {
  APIServerAvailable: {
    name: 'API Server',
    shortName: 'API',
    description: 'Indicates whether the shoot\'s kube-apiserver is healthy and available. If this is in error list then no interaction with the cluster is possible. The workload running on the cluster is most likely not affected.',
    sortOrder: '0',
  },
  ControlPlaneHealthy: {
    name: 'Control Plane',
  ...
  MaintenancePreconditionsSatisfied: {
    ...
    shortName: 'M',
    description: 'Indicates whether Gardener is able to perform required actions during maintenance. If you do not resolve this issue your cluster will eventually turn into an error list.',
    sortOrder: '5',
  },
  HibernationPossible: {
