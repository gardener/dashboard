...
import { parseKubeconfig } from '@gardener-dashboard/kube-config'
import createError, {
  UnprocessableEntity,
  NotFound,
} from 'http-errors'
import MemberManager from '../lib/services/members/MemberManager.js'
import SubjectList from '../lib/services/members/SubjectList.js'
  ...
  describe('members', function () {
    ...
    const findObjectFn = (objects) => {
      return (namespace, name) => {
        const item = find(objects, { metadata: { namespace, name } })
        if (!item) {
    ...
    beforeEach(function () {
      client['core.gardener.cloud'].projects = {
        mergePatch: vi.fn().mockResolvedValue(),
      }
      client.core.serviceaccounts = {
        create: vi.fn().mockImplementation((namespace, body) => {
          if (body.metadata.name === 'default') {
            throw createError(409)
          }
          return Promise.resolve(set(body, ['metadata', 'creationTimestamp'], 'now'))
        }),
        createTokenRequest: vi.fn().mockImplementation((namespace, name, body) => {
          return Promise.resolve(set(body, ['status', 'token'], 'secret'))
        }),
        delete: vi.fn().mockImplementation(findObjectFn(serviceAccounts)),
        mergePatch: vi.fn().mockImplementation((namespace, name, body) => {
          return Promise.resolve(set(body, ['metadata', 'creationTimestamp'], 'bar-time'))
        }),
      }
    })
