...
vi.mock('../lib/cache/index.js', async () => {
  ...
  return {
    default: {
      getProjects: vi.fn(() => projectList),
      getProject: vi.fn(name => projectList.find(project => project.metadata.name === name)),
    },
    getProjects: vi.fn(() => projectList),
    getProject: vi.fn(name => projectList.find(project => project.metadata.name === name)),
  }
...
vi.mock('@gardener-dashboard/kube-client', () => {
  ...
  return { default: mockKubeClient, ...mockKubeClient }
})

const httpErrorsMod = await import('http-errors')
const { PreconditionFailed, InternalServerError } = httpErrorsMod.default
const projects = await import('../lib/services/projects.js')
const shoots = await import('../lib/services/shoots.js')
const authorization = await import('../lib/services/authorization.js')
const { dashboardClient } = await import('@gardener-dashboard/kube-client')
...
describe('services/projects', () => {
  ...
  describe('#create', () => {
    ...
    it('should throw an error if project creation times out', async () => {
      ...
      dashboardClient['core.gardener.cloud'].projects.watch.mockResolvedValue({
        until: vi.fn().mockRejectedValue(new Error('Timeout')),
      })
