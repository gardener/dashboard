...
import { encodeBase64 } from '../lib/utils/index.js'

import {
  ensureTerminalAllowed,
  findImageDescription,
  fromShortcutSecretResource,
} from '../lib/services/terminals/index.js'
  ...
  describe('terminals', function () {
    ...
    const client = {
      ...
      async getShoot ({ namespace, name }) {
        ...
        if (namespace === 'garden' && includes([seedName, managedSeedName], name)) {
          return {
            kind: 'Shoot',
            metadata: { namespace, name },
            spec: {
    ...
    beforeEach(function () {
      terminalStub = vi.fn().mockReturnValue(terminalConfig)
    ...
    describe('index', function () {
      ...
      it('should pick only valid fields from shortcut resource', function () {
        ...
        actualShortcuts = fromShortcutSecretResource({
          data: {
            shortcuts: encodeBase64(yamlDump([
              {
                foo: 'bar',
              },
            ])),
